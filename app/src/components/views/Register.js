...
function Register() {
  ...
  const handleSubmit = () => {
    ...
    UserService.signup(values)
      .then((res) => {
        if (res.error) {
          setErrorMessage(res.error); // Set error message if signup fails
          return;
        }

        setSuccessMessage(
          "Signup successful. Please check your mailbox to verify your email address." // Set success message if signup succeeds
        );
      })
      .catch((err) => {
        console.log(err);
        ...
        <div className="row">
          ...
          <div className=" col-lg-4 col-md-6 col-12 rightSide d-md-flex">
            <div className="holder my-auto">
              ...
              {successMessage && (
                <p className="text-success mb-2">{successMessage}</p> // Display success message if any
              )}
              <p>
